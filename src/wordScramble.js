...
var WordScramble = (function () {
    ...
    WordScramble.prototype.fadeOut = function () {
        var _this = this;
        $(this.audio).animate({ volume: 0 }, 1000, "swing", function () { _this.audio.pause(); });
    };
...
function vkGetUriByAid(lesson, callback) {
    $.ajax({
        ...
        dataType: 'jsonp',
        success: function (data) { lesson.mediaUri = data.response[0].url; callback(); }
    });
}
window.onload = function () {
    ...
    var game;
    var audio = $('<audio src=resources/' + lesson.mediaUri + ' />').appendTo('body')[0];
